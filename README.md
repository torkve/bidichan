...
The repository is a plain Go module. Build with the Go toolchain you prefer:

```sh
# on this machine
/home/torkve/.ya/tools/v4/11505979923/bin/go build ./...
...
- TLS 1.2 only, with the same cipher suites stock nginx negotiates.
- The application-layer handshake is an HTTP/1.1 `Upgrade: bidichan/1`
  request. Auth is a Bearer header carrying
  `HMAC-SHA256(PSK, "client" || nonce || timestamp || tls_exporter)`.
  The server's `101 Switching Protocols` includes the matching
  server-role MAC for mutual authentication.
...
- The TLS fingerprint is Go's standard library. A JA3/JA4 inspector can
  tell a Go-tls handshake apart from a real nginx or a real browser. If
  you need to defeat that level of fingerprinting, swap in
  [uTLS](https://github.com/refraction-networking/utls); it is not
  wired up here.
